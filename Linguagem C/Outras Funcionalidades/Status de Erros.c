...
EXIT_SUCCESS >>> on POSIX pattern the value of this macro is 0
EXIT_FAILURE >>> on POSIX pattern the value of this macro is 1

exit(0) >>> It indicates not errors.
...
int main()
{
	...
	printf("Digite um numero para ver a saida de erro: ");
	scanf("%d", &status);
